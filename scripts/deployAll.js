...
async function main() {
    ...
    console.log( ohm.address + " -111- " );
    console.log( dai.address + " -222- " );
    console.log( treasury.address + " -333- ");
    //console.log( MockDAO.address + " -444- " );
    console.log( ohm.zeroAddress + " -555- " );
    const daiBond = await DAIBond.deploy(ohm.address, dai.address, treasury.address, ohm.address, zeroAddress);

    // Deploy Frax bond
    //@dev changed function call to Treasury of 'valueOf' to 'valueOfToken' in BondDepository due to change in Treausry contract
    const FraxBond = await ethers.getContractFactory('MockOlympusBondDepository');
    const fraxBond = await FraxBond.deploy(ohm.address, frax.address, treasury.address,ohm.address, zeroAddress);
